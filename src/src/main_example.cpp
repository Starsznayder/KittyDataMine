...
using ExampleTuple = std::tuple<float, float, uint8_t, uint8_t>;

std::tuple<std::vector<ExampleTuple>,
           std::vector<uint8_t>> read_tae(std::string path) {

    std::ifstream File(path);

    std::vector<ExampleTuple> data_vec;
    ...
    while (File >> cls >> d0 >> d1 >> d2 >> d3) {
        data_vec.push_back(ExampleTuple(d0, d1, (uint8_t) d2, (uint8_t) d3));
...
std::tuple<
    ...
    std::vector<uint8_t>,
    std::vector<uint8_t>>
    run_all(const std::vector<ExampleTuple>& X,
    ...
    auto metup = std::make_tuple(
        AbsoluteDifference{},
        AbsoluteDifference{},
        ValueDifference(y, nth<2, ExampleTuple>(X), 2, 4),
        ValueDifference(y, nth<3, ExampleTuple>(X), 2, 4)
    );    

    Manhattan<decltype(metup), ExampleTuple> metric(metup);
    ...
    for(size_t i = 0; i < X.size(); ++i) {
        ...
        std::vector<Observation<ExampleTuple>> dataset;
        for(size_t j = 0; j < X.size(); ++j) {
            if(j == i) continue;
            Observation<ExampleTuple> obs(j, y[j], X[j]);
...
int main() {
    ...
    // Load Train
    const auto df = read_tae("data/csv/example.csv");
    const auto X = std::get<0>(df); 
    const auto y = std::get<1>(df);

    printf("{\n");
    printf("\"file_name\": \"data/csv/example.csv\",\n");
