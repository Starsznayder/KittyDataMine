...
using TaeTuple = std::tuple<uint8_t, float, float, uint8_t, float>;

std::tuple<std::vector<TaeTuple>,
           std::vector<uint8_t>> read_tae(std::string path) {

    std::ifstream File(path);

    std::vector<TaeTuple> data_vec;
    std::vector<uint8_t> cls_vec;

    float cls;
    float d0, d1, d2, d3, d4;

    while (File >> cls >> d0 >> d1 >> d2 >> d3 >> d4) {
        data_vec.push_back(TaeTuple((uint8_t) d0, d1, d2, (uint8_t) d3, d4));
...
std::tuple<
    ...
    std::vector<uint8_t>,
    std::vector<uint8_t>>
    run_all(const std::vector<TaeTuple>& X,
            const std::vector<uint8_t>& y,
            size_t k,
            size_t number_of_classes) {

    auto metup = std::make_tuple(
        ValueDifference(y, nth<0, TaeTuple>(X), 3, 2),
        AbsoluteDifference{},
        AbsoluteDifference{},
        ValueDifference(y, nth<3, TaeTuple>(X), 3, 2),
        AbsoluteDifference{}
    );    

    Manhattan<decltype(metup), TaeTuple> metric(metup);
    ...
    for(size_t i = 0; i < X.size(); ++i) {
        ...
        std::vector<Observation<TaeTuple>> dataset;
        for(size_t j = 0; j < X.size(); ++j) {
            if(j == i) continue;
            Observation<TaeTuple> obs(j, y[j], X[j]);
...
int main() {
    ...
    // Load Train
    const auto df = read_tae("data/csv/tae.csv");
